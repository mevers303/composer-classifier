...
class VectorGetter:
    ...
    def get_chunk(self, chunk_size, train_or_test):
        ...
        y = self.y_onehot_encoder.transform(y).todense().asarray(dtype=np.byte)
    ...
    def get_all_split(self, pickle_file="", reload=True):
        ...
        y_train = self.y_onehot_encoder.transform(y_train).todense().asarray(dtype=np.byte)
        y_test = self.y_label_encoder.transform(y_test).reshape(-1, 1)
        y_test = self.y_onehot_encoder.transform(y_test).todense().asarray(dtype=np.byte)
